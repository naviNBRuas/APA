...
func (ad *AdvancedDiscovery) startDhtDiscovery(ctx context.Context) {
	...
	// Advertise ourselves
	if _, err := ad.routingDiscovery.Advertise(ctx, ad.serviceTag); err != nil {
		ad.logger.Error("Failed to advertise on DHT", "error", err)
	}
	...
	for {
		...
		case <-ctx.Done():
			...
			return
		case <-ticker.C:
			peers, err := ad.routingDiscovery.FindPeers(ctx, ad.serviceTag)
			if err != nil {
