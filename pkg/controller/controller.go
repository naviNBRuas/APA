...
// DefaultCommandFactory is the default CommandFactory using os/exec.CommandContext with light isolation.
...
// SandboxOptions define minimal process isolation knobs for controllers.
// These are best-effort and platform-dependent.
...
// SetSandboxOptions configures how the controller process is launched.
func (gbc *GoBinaryController) SetSandboxOptions(opts SandboxOptions) {
	gbc.sandbox = opts
}
...
func (gbc *GoBinaryController) Start(ctx context.Context) error {
	...
	args := []string{"--config", gbc.configFilePath, "--message-file", gbc.messageFilePath}
	gbc.cmd = gbc.CommandFactory(ctrlCtx, gbc.Manifest.Path, args...)

	// Apply sandbox options if supported by the underlying command
	if execCmd, ok := gbc.cmd.(*osExecCommand); ok {
		if gbc.sandbox.WorkingDir != "" {
			execCmd.cmd.Dir = gbc.sandbox.WorkingDir
		}
		if len(gbc.sandbox.Env) > 0 {
			execCmd.cmd.Env = gbc.sandbox.Env
		}
	}
	...
	go func() {
		...
		if gbc.cmd.Process() != nil {
			if err := gbc.cmd.Process().Kill(); err != nil {
				gbc.logger.Error("Failed to kill process", "name", gbc.name, "error", err)
			}
		}
	}()
