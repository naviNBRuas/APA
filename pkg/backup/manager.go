...
func (bm *BackupManager) CreateBackup(config, state, criticalData map[string]interface{}) (string, error) {
	...
	// Serialize backup data without checksum to compute digest
	checksumPayload, err := json.Marshal(backupData)
	if err != nil {
		return "", fmt.Errorf("failed to serialize backup data: %w", err)
	}

	// Calculate checksum on payload without the checksum field populated
	checksum := bm.calculateChecksum(checksumPayload)
	backupData.Checksum = checksum

	// Serialize final payload with checksum set
...
func (bm *BackupManager) RestoreBackup(backupFile string) (*BackupData, error) {
	...
	// Verify checksum by clearing the checksum field and re-hashing
	storedChecksum := backupData.Checksum
	backupData.Checksum = ""
	verificationPayload, err := json.Marshal(backupData)
	if err != nil {
		return nil, fmt.Errorf("failed to re-serialize backup data for verification: %w", err)
	}

	if !bm.verifyChecksum(verificationPayload, storedChecksum) {
