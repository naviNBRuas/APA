...
func (rc *RecoveryController) RequestPeerCopy(ctx context.Context, moduleName string, peerIDStr string) error {
	if rc.p2p == nil {
		return fmt.Errorf("p2p service not configured")
	}
...
func (rc *RecoveryController) verifyModule(manifest *module.Manifest, wasmBytes []byte) error {
	...
	if len(wasmBytes) == 0 {
		return fmt.Errorf("module payload is empty")
	}
	...
	if len(manifest.Hash) != 64 {
		return fmt.Errorf("module hash is not a valid SHA-256 hex string")
	}
	...
	// Opportunistic signature presence check; full signature validation would
	// require a trust store which is outside this controller's scope.
	if len(manifest.Signatures) == 0 {
		rc.logger.Warn("Module manifest missing signatures; accepting for now", "module", manifest.Name)
	}
...
func (rc *RecoveryController) QuarantineNode(ctx context.Context, nodeID string) error {
	...
	rc.mu.Lock()
	if _, exists := rc.quarantineList[nodeID]; exists {
		rc.mu.Unlock()
		rc.logger.Info("Node already quarantined", "node", nodeID)
		return nil
	}
	rc.mu.Unlock()
...
// IsQuarantined reports whether a node is currently quarantined.
func (rc *RecoveryController) IsQuarantined(nodeID string) bool {
	rc.mu.RLock()
	defer rc.mu.RUnlock()
	_, exists := rc.quarantineList[nodeID]
	return exists
}

// ListQuarantined returns a snapshot of quarantined node IDs.
func (rc *RecoveryController) ListQuarantined() []string {
	rc.mu.RLock()
	defer rc.mu.RUnlock()
	ids := make([]string, 0, len(rc.quarantineList))
	for id := range rc.quarantineList {
		ids = append(ids, id)
	}
	return ids
}
...
func (rc *RecoveryController) RestoreSnapshot(ctx context.Context) error {
	...
	// After applying the config, ensure running modules/controllers are consistent.
	if rc.moduleManager != nil {
		for _, manifest := range rc.moduleManager.ListModules() {
			...
			if err := rc.moduleManager.StopModule(manifest.Name); err != nil {
				rc.logger.Warn("Failed to stop module during restore; it will be reloaded by config", "module", manifest.Name, "error", err)
			}
		}
	}
	...
	if rc.controllerManager != nil {
		for _, manifest := range rc.controllerManager.ListControllers() {
			...
			if err := rc.controllerManager.StopController(ctx, manifest.Name); err != nil {
				rc.logger.Warn("Failed to stop controller during restore; it will be reloaded by config", "controller", manifest.Name, "error", err)
			}
		}
	}
