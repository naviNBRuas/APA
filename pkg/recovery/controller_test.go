...
func TestCreateAndRestoreSnapshot(t *testing.T) {
	...
	// Expectations for restore path
	mockModuleManager.On("ListModules").Return([]*module.Manifest{})
	mockModuleManager.On("StopModule", mock.Anything).Return(nil)
	mockControllerManager.On("ListControllers").Return([]*controllerManifest.Manifest{})
	mockControllerManager.On("StopController", mock.Anything, mock.Anything).Return(nil)
