...
func TestRBAC(t *testing.T) {
	...
	allowed, _, err := rb.Authorize(ctx, user, action, resource)
	if err != nil {
		t.Errorf("Failed to authorize user: %v", err)
	}

	if !allowed {
		t.Error("User should be authorized")
	}

	// Test authorizing a user without permission
	action2 := "write"
	allowed, _, err = rb.Authorize(ctx, user, action2, resource)
	if err != nil {
		t.Errorf("Failed to authorize user: %v", err)
	}
	...
	allowed, _, err = rb.Authorize(ctx, "bob", action, resource)
	if err != nil {
		t.Errorf("Failed to authorize user: %v", err)
	}
