...
func TestComprehensiveInjectionIntegration(t *testing.T) {
	...
	if err := advancedInjector.RequestInjection("test-process"); err != nil {
		t.Logf("Advanced injection request failed (expected in test): %v", err)
	}
...
func TestInjectionWithNonexistentPayloadComprehensive(t *testing.T) {
	...
	_ = advancedInjector.RequestPayloadInjection("test-process", "/nonexistent/payload")
